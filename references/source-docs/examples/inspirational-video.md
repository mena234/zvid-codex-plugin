---
source_url: "https://docs.zvid.io/docs/examples/inspirational-video"
sidebar_position: 1
---

# Inspirational Video

This example is a 28-second HD motivational video built from six `VIDEO` scenes, two audio tracks, and karaoke-style subtitles.
...
The scenes are linked with `id` and `transitionId`:
...
Audio uses one narration track and one quiet background music track:
...
Subtitles use karaoke mode with word-level timing:
...
Each scene is a `VIDEO` element positioned on the timeline with `enterBegin` and `exitEnd`. The source clips are muted with `volume: 0`, then narration and music are added through the root `audios` array.

The first five scenes declare transitions to the next scene:

| Scene                  | Transition   | Duration |
| ---------------------- | ------------ | -------- |
| `scene-1` to `scene-2` | `fade`       | `0.6`    |
| `scene-2` to `scene-3` | `smoothleft` | `0.7`    |
| `scene-3` to `scene-4` | `fadeblack`  | `0.6`    |
| `scene-4` to `scene-5` | `dissolve`   | `0.6`    |
| `scene-5` to `scene-6` | `fadewhite`  | `0.7`    |
...
<details>
<summary>Show full JSON config</summary>
...
- Use `id` and `transitionId` to link video scenes.
- Set source video `volume` to `0` when narration should drive the audio.
- Karaoke subtitles require word-level `start` and `end` timestamps.
- Keep background music low, often around `0.1` to `0.3`, so it does not compete with narration.
